...
exports.handler = async function (event) {
  ...
  const { to, name, space, eventDate, status, quotedRate, requesterEmail, requesterPhone, guestCount, eventType, notes } = data;
  ...
  if (status === 'new_request') {
    subject = `New Rental Request — ${space}`;
    bodyHtml = `
      <p>A new rental request just came in through the website:</p>
      <table style="border-collapse:collapse;">
        <tr><td style="padding:4px 12px 4px 0; font-weight:bold;">Name:</td><td style="padding:4px 0;">${name}</td></tr>
        <tr><td style="padding:4px 12px 4px 0; font-weight:bold;">Email:</td><td style="padding:4px 0;">${requesterEmail || ''}</td></tr>
        <tr><td style="padding:4px 12px 4px 0; font-weight:bold;">Phone:</td><td style="padding:4px 0;">${requesterPhone || ''}</td></tr>
        <tr><td style="padding:4px 12px 4px 0; font-weight:bold;">Space:</td><td style="padding:4px 0;">${space}</td></tr>
        <tr><td style="padding:4px 12px 4px 0; font-weight:bold;">Event Date:</td><td style="padding:4px 0;">${formattedDate}</td></tr>
        <tr><td style="padding:4px 12px 4px 0; font-weight:bold;">Guest Count:</td><td style="padding:4px 0;">${guestCount || ''}</td></tr>
        <tr><td style="padding:4px 12px 4px 0; font-weight:bold;">Event Type:</td><td style="padding:4px 0;">${eventType || ''}</td></tr>
        <tr><td style="padding:4px 12px 4px 0; font-weight:bold; vertical-align:top;">Notes:</td><td style="padding:4px 0;">${notes || '—'}</td></tr>
      </table>
      <p>Log in to the staff dashboard to review and respond.</p>
    `;
